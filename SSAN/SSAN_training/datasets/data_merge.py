    ...
    def get_single_dataset(self, data_name="", train=True, img_size=256, map_size=32, transform=None, debug_subset_size=None, UUID=-1):
        if train:
            ...
            if data_name in ["OULU"]:
                data_set = Spoofing_train_oulu(os.path.join(data_dir, "train_list_video.txt"),
                                               os.path.join(data_dir, "Train_files"), transform=transform,
                                               img_size=img_size, map_size=map_size, UUID=UUID)
            elif data_name in ["private"]:
                data_set = Spoofing_train_private(os.path.join("/root/data_train.txt"),
                                               os.path.join(data_dir, "Train_files"), transform=transform,
            ...
            if data_name in ["OULU"]:
                ...
                data_set = Spoofing_valtest_private(os.path.join("/root/test_FA.txt"),
                                               os.path.join(data_dir, "Train_files"), transform=transform,
                                               img_size=img_size, map_size=map_size)
            elif data_name in ["CASIA_MFSD", "Replay_attack", "MSU_MFSD"]:
