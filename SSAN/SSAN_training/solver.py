...
def main(args):
    ...
    writer = SummaryWriter(args.log + "/" + args.log + "_SSAN_{:04d}_{:02d}_{:02d}".format(year, month, day))
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for sample_batched in tqdm.tqdm(train_loader):
            ...
            if i % args.print_freq == args.print_freq - 1:
                ...
                #         loss_record.avg))
                log_file.write(
                    "epoch:{:d}, mini-batch:{:d}, lr={:.4f}, binary_loss={:.4f}, constra_loss={:.4f}, adv_loss={:.4f}, "
                    "Loss={:.4f}\n".format(
                        epoch + 1, i + 1, lr, binary_loss_record.avg, constra_loss_record.avg, adv_loss_record.avg,
                        loss_record.avg))
                log_file.flush()
                writer.add_scalar('binary_loss',
                                  binary_loss_record.avg,
                                  epoch * len(train_loader) + i)

                writer.add_scalar('adv_loss',
                                  adv_loss_record.avg,
                                  epoch * len(train_loader) + i)
            i = i + 1
        ...
        if epoch % epoch_test == epoch_test - 1:
            ...
            for i, test_name in enumerate(test_data_dic.keys()):
                ...
                test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=64)
