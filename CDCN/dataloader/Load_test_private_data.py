...
class Normaliztion_valtest(object):
    ...
    def __call__(self, sample):
        image_x, spoofing_label, spoofing_name, path_name = sample['image_x'],sample['spoofing_label'], sample['spoofing_name'], sample['path']
        new_image_x = (image_x - 127.5)/128     # [-1,1]
#        new_image_x = image_x/255.0

        return {'image_x': new_image_x, 'spoofing_label': spoofing_label, 'spoofing_name': spoofing_name, 'path': path_name}
...
class ToTensor_valtest(object):
    ...
    def __call__(self, sample):
        ...
        return {'image_x': torch.from_numpy(image_x.astype(np.float)).float(),'spoofing_label': torch.from_numpy(spoofing_label_np.astype(np.long)).long(),'spoofing_name':spoofing_name,'path': path_name} 
...
class Spoofing_valtest(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "live" in image_path:
            val_map_path = image_path.replace("crop", "depth")
            image_x, val_map_x = self.get_single_image_x(image_path, val_map_path, flag=True)
            spoofing_label = 1
        else:
            val_map_path = " "
            image_x = self.get_single_image_x(image_path, val_map_path, flag=False)# , videoname)
            spoofing_label = 0
            val_map_x = np.zeros((32, 32))

        sample = {'image_x': image_x, 'spoofing_label': spoofing_label, "spoofing_name":  file_imgs[len(file_imgs)-3], "path": self.landmarks_frame[idx][0] }

        if self.transform:
            sample = self.transform(sample)
        return sample

    def get_single_image_x(self, image_path, val_map_path, flag=True):
        # RGB
        image_x_temp = cv2.imread(image_path)
        # gray-map
        image_x = resize_and_pad(image_x_temp, (self.size, self.size))
